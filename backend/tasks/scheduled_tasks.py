...
def send_daily_reminders():
    ...
    with app.app_context():
        ...
        pending_requests = ServiceRequest.query.filter(service_status = 'pending').all()

        professionals_to_notify = {}

        for req in pending_requests:
            if req.proffesional_id and req.proffesional_id not in professionals_to_notify:
                professionals_to_notify[req.proffesional_id] = []
            
            professionals_to_notify[req.proffesional_id].append({
                'request_id': req.id,
        ...
        for prof_id, req in professionals_to_notify.items():
            prof = Professional.query.get(prof_id)
            if prof and prof.user:
                send_remainder_notification(prof.user, req)


def send_remainder_notification(user, req):
